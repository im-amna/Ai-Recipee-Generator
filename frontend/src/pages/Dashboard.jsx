...
import { dummyRecipes, dummyPantryItems, dummyMealPlans } from '../data/dummyData';
...
const Dashboard = () => {
    ...
    useEffect(() => {
        if (isDemo) {
            setStats({
                totalRecipes: dummyRecipes.length,
                pantryItems: dummyPantryItems.length,
                mealsThisWeek: dummyMealPlans.length
            });
            setRecentRecipes(dummyRecipes.slice(0, 5));
            setUpcomingMeals(dummyMealPlans.slice(0, 5));
            setLoading(false);
        } else {
            fetchDashboardData();
        }
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                {isDemo && (
                    <div className="mb-6 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 text-emerald-700 text-sm">
                        👋 You're viewing a demo — <Link to="/signup" className="font-semibold underline">Sign up free</Link> to save your own recipes.
                    </div>
                )}
                <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <Link to="/generate" className="bg-linear-to-r from-emerald-50 to-emerald-100 text-emerald-500 p-6 rounded-xl shadow-sm hover:shadow-md transition-all group">
                    ...
                    <Link to="/pantry" className="bg-white border border-gray-200 p-6 rounded-xl shadow-sm hover:shadow-md transition-all group">
                        <div className="flex items-center gap-4">
                        ...
                        {recentRecipes.length > 0 ? (
                            <div className="space-y-3">
                                {recentRecipes.map((recipe) => (
                                    <Link key={recipe.id} to={`/recipes/${recipe.id}`} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-sm text-gray-500 flex items-center gap-1">
                                                <Clock className="w-3 h-3" />{recipe.cook_time} mins
                                            </p>
                                        </div>
                                    </Link>
                                ))}
